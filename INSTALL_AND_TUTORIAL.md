...
Requested analysis completed successfully

1

Partial result, such as a plotting failure or rejected/failed minimization

2

Configuration error or no usable structure for the requested analysis source
...
Run contact-only analysis on all original structures.

Inspect qc.tsv, clash counts, contact consistency, and predicted-model
confidence information from the original modeling workflow.

Prepare exact ligand chemistry as a curated SDF.

Run restrained minimization while retaining original-coordinate analysis.

Inspect every rejected candidate and the before/after QC figure.

Run the accepted-minimized analysis into a separate output directory.

Use Vina only for rapid triage; use consistently sampled MM/GBSA for an
approximate energetic comparison or RBFE for a suitable congeneric series.

Treat experimental Kd under a controlled assay protocol as the decision
standard.
