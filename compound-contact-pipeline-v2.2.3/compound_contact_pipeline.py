...
class DatasetSpec:
    ...
    ligand_compound_id: str | None = None
    ligand_chemistry_source: str | None = None
...
def load_smiles_table(path: Path) -> dict[str, str]:
    """Load a one-to-one Compound ID to SMILES mapping from TSV or CSV."""
    path = Path(path).resolve()
    if not path.is_file():
        raise PipelineError(f"SMILES table is not a file: {path}")
    delimiter = "," if path.suffix.lower() == ".csv" else "\t"
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        reader = csv.DictReader(handle, delimiter=delimiter)
        normalized = {
            _normalized_table_header(name): name for name in (reader.fieldnames or ())
        }
        missing = [name for name in ("compoundid", "smiles") if name not in normalized]
        if missing:
            raise PipelineError(
                "SMILES table requires columns named Compound ID and Smiles"
            )
        id_column = normalized["compoundid"]
        smiles_column = normalized["smiles"]
        entries: dict[str, str] = {}
        for row_number, row in enumerate(reader, start=2):
            compound_id = (row.get(id_column) or "").strip()
            smiles = (row.get(smiles_column) or "").strip()
            if not compound_id and not smiles:
                continue
            if not compound_id or not smiles:
                raise PipelineError(
                    f"SMILES table row {row_number}: Compound ID and Smiles "
                    "must both be populated"
                )
            if compound_id in entries:
                raise PipelineError(
                    f"SMILES table row {row_number}: duplicate Compound ID "
                    f"{compound_id!r}"
                )
            entries[compound_id] = smiles
    if not entries:
        raise PipelineError("SMILES table contains no compound entries")
    return entries


def apply_smiles_table(
    jobs: list[StructureJob], table_path: Path
) -> list[StructureJob]:
    """Fill unresolved job chemistry from IDs embedded in job paths/names."""
    entries = load_smiles_table(table_path)
    source = f"smiles_table:{Path(table_path).resolve().as_posix()}"
    resolved: list[StructureJob] = []
    patterns = {
        compound_id: re.compile(
            rf"(?<![A-Za-z0-9]){re.escape(compound_id)}(?![A-Za-z0-9])",
            re.IGNORECASE,
        )
        for compound_id in entries
    }
    for job in jobs:
        if job.ligand_smiles or job.ligand_file:
            resolved.append(job)
            continue
        haystack = "\n".join((job.compound, job.dataset_rel, job.structure_id))
        matches = sorted(
            compound_id
            for compound_id, pattern in patterns.items()
            if pattern.search(haystack)
        )
        if not matches:
            raise PipelineError(
                f"no Compound ID from {Path(table_path).name!r} matches "
                f"{job.structure_id!r}"
            )
        if len(matches) > 1:
            raise PipelineError(
                f"multiple compound IDs from {Path(table_path).name!r} match "
                f"{job.structure_id!r}: {', '.join(matches)}"
            )
        compound_id = matches[0]
        resolved.append(
            replace(
                job,
                ligand_smiles=entries[compound_id],
                ligand_compound_id=compound_id,
                ligand_chemistry_source=source,
            )
        )
    return resolved
        ...
        for row_number, row in enumerate(reader, start=2):
            ...
            specs.append(
                DatasetSpec(
                    ...
                    ligand_chemistry_source=(
                        "manifest_smiles"
                        if ligand_smiles
                        else "manifest_ligand_file"
                        if ligand_file
                        else None
                    ),
                )
...
def _job_from_spec(root: Path, cif_path: Path, spec: DatasetSpec) -> StructureJob:
    return StructureJob(
        ...
        ligand_compound_id=spec.ligand_compound_id,
        ligand_chemistry_source=spec.ligand_chemistry_source,
    )
            ...
            for cif_path in sorted(
                ...
                jobs.append(
                    StructureJob(
                        ...
                        ligand_chemistry_source=(
                            "command_line_smiles"
                            if default_ligand_smiles
                            else "command_line_ligand_file"
                            if default_ligand_file
                            else None
                        ),
                    )
                )
...
def calculate_buried_sasa_biopython_reference(
    protein_residues, ligand_residues
) -> tuple[float, float]:
    """Reference SASA implementation retained for validation tests."""
...
def _load_freesasa():
    """Import the required compiled FreeSASA Python backend."""
    try:
        return importlib.import_module("freesasa")
    except ImportError as exc:
        raise PipelineError(
            "FreeSASA is required for SASA analysis in v2.2.3; install "
            "the conda-forge package 'freesasa-python>=2.2' "
            "(for example: conda install -c conda-forge 'freesasa-python>=2.2')"
        ) from exc


def _sasa_backend_version() -> str:
    freesasa = _load_freesasa()
    return str(getattr(freesasa, "__version__", "unknown"))


def _sasa_backend_description() -> str:
    return f"FreeSASA {_sasa_backend_version()} Lee-Richards; 20 slices; 1 thread/worker"


def _freesasa_coordinates_and_radii(residues) -> tuple[list[float], list[float]]:
    coords: list[float] = []
    radii: list[float] = []
    for residue in residues:
        for atom in residue.get_atoms():
            ...
            radii.append(float(ATOMIC_RADII[element]))
    return coords, radii


def _freesasa_total_area(residues, freesasa, parameters) -> float:
    coords, radii = _freesasa_coordinates_and_radii(residues)
    if not radii:
        return 0.0
    return float(freesasa.calcCoord(coords, radii, parameters).totalArea())


def calculate_buried_sasa(
    protein_residues, ligand_residues
) -> tuple[float, float]:
    """Calculate buried SASA with compiled FreeSASA Lee-Richards."""
    freesasa = _load_freesasa()
    parameters = freesasa.Parameters()
    parameters.setProbeRadius(1.4)
    parameters.setNSlices(20)
    parameters.setNThreads(1)
    receptor_sasa = _freesasa_total_area(protein_residues, freesasa, parameters)
    ligand_sasa = _freesasa_total_area(ligand_residues, freesasa, parameters)
    complex_sasa = _freesasa_total_area(
        [*protein_residues, *ligand_residues], freesasa, parameters
    )
    buried_total = max(0.0, receptor_sasa + ligand_sasa - complex_sasa)
    return buried_total, buried_total / 2.0
    ...
    return [
        {
            ...
            "ligand_compound_id": job.ligand_compound_id,
            "ligand_chemistry_source": job.ligand_chemistry_source,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "TSV/CSV with Compound ID and Smiles columns; fills chemistry "
            "when an ID is embedded in a compound folder or structure path"
        ),
    )
    parser.add_argument(
        "--openmm-platform",
...
def _analyze_jobs(
    ...
    *,
    ...
    reporter = reporter or ProgressReporter(quiet=True)
    ...
    sasa_status = (
        f"enabled ({_sasa_backend_description()})" if compute_sasa else "disabled"
    )
    reporter.info(
        f"{len(jobs)} structures | {args.workers} CPU workers | SASA {sasa_status}"
    )

    if args.workers == 1:
        results: list[StructureResult] = []
        width = len(str(len(jobs))) if jobs else 1
        for completed, job in enumerate(jobs, start=1):
            _, result, elapsed = _timed_analyze_job(
                completed - 1,
                job,
                args.cutoff,
                args.clash_cutoff,
...
def _openmm_progress_callback(
    ...
    def callback(event) -> None:
        suffix = f" [{platform}]" if event.operation in stage_operations else ""
        if event.state == "started":
            ...
            return
        label = "preparation" if event.operation == "preparation" else event.operation
...
def run_minimization_jobs(
    ...
    def report_outcome(record: MinimizationRecord, item_started: float) -> None:
        reporter.info(f"    {record.status.upper()}")
        if record.reasons:
            reporter.info(f"    reason: {'; '.join(record.reasons)}")
        reporter.info(
    ...
    for job, original in zip(jobs, original_results):
        ...
        if original.status != "valid":
            ...
            report_outcome(record, item_started)
            continue
        ...
        record = MinimizationRecord(
            ...
            package_versions=(
                *backend.package_versions,
                ("freesasa", _sasa_backend_version()),
            ),
        )
        records.append(record)
        report_outcome(record, item_started)
...
def _analyze_accepted_minimized(
    ...
    if len(jobs) != len(minimization_records):
...
def select_analysis_results(
    ...
    if len(jobs) != len(original_results) or len(jobs) != len(minimization_records):
        raise PipelineError("minimization records must align with structure jobs")
    if full_minimized_results is None or len(full_minimized_results) != len(jobs):
        raise PipelineError(
            "full minimized results must align with structure jobs for minimized analysis"
        )

    selected: list[StructureResult] = []
    for index, (job, original, record) in enumerate(
        zip(jobs, original_results, minimization_records)
    ):
        if record.status == "accepted" and record.candidate_result is not None:
            full_result = full_minimized_results[index]
            if full_result is None:
                raise PipelineError(
                    f"accepted minimized structure lacks full analysis: {job.structure_id}"
                )
            selected.append(full_result)
            continue
        stage = (
...
def _run_pipeline(
    ...
    if args.smiles_table:
        jobs = apply_smiles_table(jobs, args.smiles_table)
    ...
    if args.minimize:
        ...
        if args.analysis_source == "minimized":
            full_minimized_results = _analyze_accepted_minimized(
                jobs,
        ...
        results = select_analysis_results(
            ...
            minimization_records,
            args,
            full_minimized_results=full_minimized_results,
        )
    else:
    ...
    if args.analysis_source == "minimized" and not valid_results:
        ...
        summary_path = claim_output_path(
            output_dir / "minimization_summary.tsv", output_dir
        )
        write_tsv(
            summary_path,
            minimization_records_to_rows(minimization_records, original_results),
            MINIMIZATION_SUMMARY_FIELDS,
        )
        print(summary_path)
