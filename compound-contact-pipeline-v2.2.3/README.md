...
- Matplotlib
- Biopython
- FreeSASA Python bindings (`freesasa` import; `freesasa-python` on conda-forge)
...
On Linux, the conda-forge package is recommended if a local compiler is not
available:

```bash
conda install -c conda-forge "freesasa-python>=2.2"
```
...
The expected version for this release is `2.2.3`.
...
## v2.2.3 accelerated SASA backend

v2.2.3 uses the compiled FreeSASA C backend with the Lee-Richards algorithm.
The production calculation uses a 1.4 A probe, 20 slices per atom, the same
element-based atomic radii used by the Biopython reference implementation, and
one FreeSASA thread per worker process. With `--workers 16`, up to 16 accepted
structures can therefore run SASA concurrently without nested threading.

The progress header reports the backend explicitly, for example:

```text
25 structures | 16 CPU workers | SASA enabled (FreeSASA 2.2.1 Lee-Richards; 20 slices; 1 thread/worker)
```

Lee-Richards and Shrake-Rupley are different numerical SASA algorithms. Treat
v2.2.3 as a new quantitative SASA baseline for structural SAR: recompute all
compounds with v2.2.3 rather than mixing v2.2.1/v2.2.2 SASA values in one
comparison. The buried-SASA definition itself remains receptor SASA + ligand
SASA - complex SASA, with `interface_area_A2 = buried_sasa_total_A2 / 2`.

A benchmark helper is included to compare one representative structure against
the retained Biopython Shrake-Rupley reference:

```bash
python benchmark_sasa.py model.cif \
  --ligand LIG_F \
  --protein-chain A,B,C \
  --ligand-chain F \
  --repeats 1
...
The production SASA backend is FreeSASA's compiled Shrake–Rupley
implementation with a 1.4 Å probe, 100 surface points, one internal thread per
worker process, and Biopython's element-based atomic radii. The script reports:
