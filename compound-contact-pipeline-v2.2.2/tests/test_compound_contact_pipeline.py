...
def test_version_is_2_2_2():
    assert ccp.__version__ == "2.2.2"
...
def test_sasa_backend_description_reports_version_and_threading(monkeypatch):
    ...
    assert ccp._sasa_backend_description() == "FreeSASA 2.2.1; 1 thread/worker"
...
def test_cli_version_outputs_pipeline_version(capsys):
    ...
    assert capsys.readouterr().out == "2.2.2\n"
...
def test_freesasa_backend_uses_biopython_radii_and_single_thread(monkeypatch):
    calls = []
    parameter_dicts = []
    ...
    class FakeParameters:
        def __init__(self, values):
            parameter_dicts.append(dict(values))
