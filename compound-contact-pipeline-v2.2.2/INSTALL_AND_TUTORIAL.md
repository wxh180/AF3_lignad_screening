...
This guide explains how to install and run version 2.2.2 of the compound
...
### FreeSASA requirement and v2.2.2 SASA acceleration

v2.2.2 requires the FreeSASA Python bindings for all full scientific analyses.
...
python - <<'PY'
import freesasa
print(getattr(freesasa, "__version__", "unknown"))
PY
```

The production calculation uses FreeSASA's compiled Shrake-Rupley backend with
a 1.4 Å probe, 100 surface points, the same element-based atomic radii as the
former Biopython calculation, and one FreeSASA thread per worker process.
With `--workers 16`, up to 16 structures are analyzed concurrently without
nested FreeSASA threading. Treat v2.2.2 SASA values as a new quantitative
baseline and recompute all compounds before structural-SAR comparisons.
...
minimized structures receive the final full SASA calculation. In v2.2.2 that
final calculation uses compiled FreeSASA. There is no `--sasa-mode` option.
