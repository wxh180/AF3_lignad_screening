...
The expected version for this release is `2.2.2`.
...
## v2.2.2 accelerated SASA backend

v2.2.2 replaces the production Biopython Shrake-Rupley loop with the compiled
FreeSASA C backend. The calculation still uses a 1.4 Å probe, 100
Shrake-Rupley surface points, and the same element-based atomic radii used by
Biopython. Each worker forces FreeSASA to one internal thread so that
`--workers 16` means up to 16 SASA processes rather than nested threading.

The progress header reports the active backend and version, for example:

```text
25 structures | 16 CPU workers | SASA enabled (FreeSASA 2.2.1; 1 thread/worker)
```

Because FreeSASA and Biopython use different compiled/Python implementations
and may place Shrake-Rupley test points differently, treat v2.2.2 as a new
quantitative SASA baseline. Do not mix v2.2.1 and v2.2.2 SASA values in the
same structural-SAR comparison; recompute all compounds with one backend.
