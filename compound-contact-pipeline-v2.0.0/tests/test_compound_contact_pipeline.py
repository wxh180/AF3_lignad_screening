...
def test_cli_rejects_minimization_with_multiple_workers(tmp_path):
    args = ccp.build_parser().parse_args(
        [str(tmp_path), "--minimize", "openmm", "--workers", "2"]
    )
...
def fake_minimization_backend(*, ligand_x=2.5, ligand_rmsd_A=0.5, fail=None):
    def minimize(request, config, cache_path):
        if fail is not None:
            raise fail
        write_test_cif(
            request.output_path,
...
def test_minimization_summary_formats_completed_metrics_and_versions(tmp_path):
    _, args, jobs, original_results = minimization_fixture(tmp_path)
    records = ccp.run_minimization_jobs(
        jobs,
        original_results,
    ...
    assert row["package_versions"] == "openmm=test"
