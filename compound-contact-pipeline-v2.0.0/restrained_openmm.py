...
def _ligand_hydrogens(topology, ligand, molecule):
    ...
    message = ("ligand element/bond graph does not match supplied chemistry (or missing "
               "heavy atoms); regenerate mmCIF with explicit _struct_conn records")
...
def minimize_structure(
    ...
    try:
        return _minimize_structure(request, config, cache_path, stack)
...
def _minimize_structure(request, config, cache_path, stack):
