...
The expected version for this release is `2.0.0`.
...
--workers N                  deterministic process count; default 1
...
Minimization is deliberately sequential (`--workers 1`) because parameter
caches, device selection, and per-structure failure reporting must remain
deterministic.
...
| Output | Contents |
|---|---|
| `run_manifest.tsv` | Resolved assignments plus original/analysis source paths |
